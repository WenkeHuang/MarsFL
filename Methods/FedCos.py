...
class FedCos(FederatedMethod):
    NAME = 'FedCos'

    def __init__(self, nets_list, client_domain_list, args, cfg):
        super(FedCos, self).__init__(nets_list, client_domain_list, args, cfg)
    ...
    def get_fc_weight(self, domain, online_clients_list):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(self.train_eval_loaders[domain]):
                ...
                # cos相似度
                loss_cos = 1 - torch.nn.CosineSimilarity(dim=1)(F.normalize(logits_0), F.normalize(logits_1))
                weighted_loss = loss_cos * ins_weight
                # 计算加权损失的平均值
