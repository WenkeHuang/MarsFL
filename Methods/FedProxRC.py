...
class FedProxRC(FederatedMethod):
    NAME = 'FedProxRC'

    def __init__(self, nets_list, client_domain_list, args, cfg):
        super(FedProxRC, self).__init__(nets_list, client_domain_list, args, cfg)
    ...
    def train_net(self, index, net, train_loader):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                optimizer.zero_grad()
                iterator.desc = "Local Pariticipant %d ce = %0.3f" % (index, loss_ce)
                loss_ce.backward()
                optimizer.step()
