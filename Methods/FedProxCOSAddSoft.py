...
class FedProxCOSAddSoft(FederatedMethod):
    NAME = 'FedProxCOSAddSoft'

    def __init__(self, nets_list, client_domain_list, args, cfg):
        super(FedProxCOSAddSoft, self).__init__(nets_list, client_domain_list, args, cfg)
        self.mu = cfg[self.NAME].mu
        self.alpha = cfg[self.NAME].alpha
        self.beta = cfg[self.NAME].beta
        self.smooth=cfg[self.NAME].smooth
    ...
    def update_head(self, index, net, train_loader):
        ...
        criterion = nn.CrossEntropyLoss(label_smoothing=self.smooth)
        criterion.to(self.device)
        iterator = tqdm(range(self.cfg.OPTIMIZER.local_epoch))
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                loss = loss_ce
                optimizer.zero_grad()
                iterator.desc = "Local Unbiased FC %d ce = %0.3f" % (index, loss_ce)
                loss.backward()
                optimizer.step()
