...
class FedProxCOS(FederatedMethod):
    NAME = 'FedProxCOS'

    def __init__(self, nets_list, client_domain_list, args, cfg):
        super(FedProxCOS, self).__init__(nets_list, client_domain_list, args, cfg)
        self.mu = cfg[self.NAME].mu # FedProx的默认参数
        self.alpha = cfg[self.NAME].alpha # 分布损失的权重
        self.beta = cfg[self.NAME].beta # 熵损失的权重
        self.w = cfg[self.NAME].w # 非目标蒸馏损失的权重
        self.temperature = cfg[self.NAME].temperature # 非目标的权重
    ...
    def train_net(self, index, net, train_loader):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                optimizer.zero_grad()
                iterator.desc = "Local Client %d ce = %0.3f" % (index, loss_ce)
                loss_ce.backward()
                optimizer.step()
    ...
    def get_fc_weight(self, domain, online_clients_list):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(self.train_eval_loaders[domain]):
                ...
                weighted_loss = loss_cos * ins_weight # 使用unsqueeze将权重扩展到与损失相同的维度
