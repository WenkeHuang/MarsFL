...
class FedR(FederatedMethod):
    NAME = 'FedR'

    def __init__(self, nets_list, client_domain_list, args, cfg):
        super(FedR, self).__init__(nets_list, client_domain_list, args, cfg)
    ...
    def get_fc_weight(self, domain, online_clients_list):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(self.train_eval_loaders[domain]):
                ...
                # 计算logits
                logits_0 = func_g(new_g_para, f_0) # 弱分支
                logits_1 = func_g(new_g_para, f_1) # 强分支
