...
class FedProxRCO(FederatedMethod):
    NAME = 'FedProxRCO'
    '''
    调用原本的分支，这样效果不太行
    '''
    def __init__(self, nets_list, client_domain_list, args, cfg):
        super(FedProxRCO, self).__init__(nets_list, client_domain_list, args, cfg)
    ...
    def train_net(self, index, net, train_loader):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                optimizer.zero_grad()
                iterator.desc = "Local participant %d ce = %0.3f dkd = %0.3f" % (index, loss_ce, nckd_loss)
                loss.backward()
                optimizer.step()
        net.train()
        sub_fc.eval()
