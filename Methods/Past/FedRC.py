...
class FedRC(FederatedMethod):
    NAME = 'FedRC'

    def __init__(self, nets_list, client_domain_list, args, cfg):
        super(FedRC, self).__init__(nets_list, client_domain_list, args, cfg)
    ...
    def get_fc_weight(self, domain, online_clients_list):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(self.train_eval_loaders[domain]):
                ...
                # kl相互拉近
                # kl1 = F.kl_div(F.log_softmax(logits_0, dim=1), F.softmax(logits_1, dim=1), reduction='batchmean')
                # kl2 = F.kl_div(F.log_softmax(logits_1, dim=1), F.softmax(logits_0, dim=1), reduction='batchmean')
                # loss_ins = F.mse_loss(F.softmax(logits_0, dim=1),F.softmax(logits_1, dim=1))
                # loss_ins = F.mse_loss(logits_0,logits_1)
                # loss_ins = (kl1 + kl2) / 2

                # 计算每个样本的MSE损失，并应用权重
                loss_mse = F.mse_loss(logits_0, logits_1, reduction='none')  # 'none'选项保留每个样本的损失
                weighted_loss = loss_mse * ins_weight.unsqueeze(1)  # 使用unsqueeze将权重扩展到与损失相同的维度
