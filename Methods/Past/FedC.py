...
class FedC(FederatedMethod):
    NAME = 'FedC'

    def __init__(self, nets_list, client_domain_list, args, cfg):
        super(FedC, self).__init__(nets_list, client_domain_list, args, cfg)
    ...
    def get_fc_weight(self, domain, online_clients_list):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(self.train_eval_loaders[domain]):
                ...
                with torch.no_grad():
                    logits_aim = func_g(new_g_para, f_1)  # 强增强

                # kl 弱aug往强的拉
                loss_kl = F.kl_div(F.log_softmax(logits_0.detach(), dim=1), F.softmax(logits_aim, dim=1), reduction='none')
