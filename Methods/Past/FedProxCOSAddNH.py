...
class FedProxCOSAddNH(FederatedMethod):
    NAME = 'FedProxCOSAddNH'

    def __init__(self, nets_list, client_domain_list, args, cfg):
        super(FedProxCOSAddNH, self).__init__(nets_list, client_domain_list, args, cfg)
    ...
    def update_head(self, index, net, train_loader):
        ...
        for _ in iterator:
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                # loss_mad = self.mad(personalized_fc,sub_fc)
                # loss = loss_ce + self.w * loss_mad
                optimizer.zero_grad()
