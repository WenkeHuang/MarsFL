...
class Conv_Net_Wrap(LoRALayer):
    ...
    def conv1x1(input_channel, output_channel, bias=False):
        ...
        return Conv2d(input_channel, output_channel, kernel_size=1, bias=bias, r=lora_param.r,
    ...
    def conv3x3(in_channel, out_channel, stride=1, padding=1, bias=False):
        lora_param = Conv_Net_Wrap.LORA_PARAM_MODEL
        return Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=padding, bias=bias, r=lora_param.r,
                      lora_alpha=lora_param.lora_alpha, lora_dropout=lora_param.lora_dropout)
#
#
def conv1x1(input_channel, output_channel):
    # return nn.Conv2d(input_channel, output_channel, kernel_size=1, bias=bias)
    return Conv_Net_Wrap.conv1x1(input_channel, output_channel)
...
def conv3x3(in_channel, out_channel, stride=1, padding=1):
    ...
    return Conv_Net_Wrap.conv3x3(in_channel, out_channel, stride=stride, padding=padding)
#
#
...
class ResNet(nn.Module):
    ...
    # def train(self, mode):
    #     if mode not in ('lora','bone',False):
    #         raise ValueError('not valid mode !')

    #     # 两个部分的训练
    #     if mode in ('lora','bone'):
    #         self.training = True
    #         for module in self.children():
    #             if isinstance(module,LoRALayer):
    #                 # LoraLayer需要根据mode来判断是否进行merge weight
    #                 module.train(mode)
    #             else:
    #                 # 普通module的train函数是不支持str类型
    #                 module.train(True)
    #         return self
...
def resnet10_lora(cfg):
    ...
    if cfg.DATASET.pretrained:
        ...
        maped_dict = map_pretrianed_model_state(pretrained_dict)
        model.load_state_dict(maped_dict, strict=False)
    return model


def resnet18_lora(cfg):
    '''Lora setting'''
    # 4 1 0
    Conv_Net_Wrap.set_conv_net_lora_param(cfg.DATASET.rank, cfg.DATASET.lora_alpha, cfg.DATASET.lora_dropout)
...
def map_pretrianed_model_state(pretrained_dict):
    maped_dict = {}
    for k, v in pretrained_dict.items():
        if 'layer' in k and 'conv' in k:
            if 'weight' in k:
                temp = k[:-6] + 'conv.weight'
            elif 'bias' in k:
                temp = k[:4] + 'conv.bias'
            maped_dict[temp] = v
            continue
